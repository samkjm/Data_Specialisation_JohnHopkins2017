...
The mean of distribution based on simulations is `r MeanSample` and is close to the theoretical value of `r MeanTheory` for lambda = 0.2. 
...
```
The theoretical variance is `r VarTheory`, whereas the variance of the means is `r VarSample`
...
The blue Theoretical Curve is based on the theoretical mean and variance of `r MeanTheory` and `r VarTheory` respectivaly. The red Simulation-Based Normal Curve is based on the simulation mean and variance of `r MeanSample` and `r VarSample` respectively. The green Simulation Curve is simply based on actual simulation values converted to density.
...
labs(title="Distribution of Means ", y="Density", x="Spread")+
stat_function(fun=dnorm,args=list( mean=1/lambda, sd=sqrt(VarTheory)),color = "red", size = 1.0)+
stat_function(fun=dnorm,args=list( mean=MeanSample, sd=sqrt(VarSample)),color = "black", size = 1.0)
